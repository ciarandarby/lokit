...
def test_tmx_to_dict_is_lazy_flat_stable_and_preserves_native_raw_tags(
    ...
    assert sanitized[0] == {
        "source_language": "en",
        "target_language": "fr",
        "source": "Hello {LOKIT_P1}world {LOKIT_P2}{LOKIT_P3}!",
        "target": "{LOKIT_P1}Bonjour{LOKIT_P2}",
        "domain": "checkout",
    }
...
def test_namespaced_raw_xliff_survives_native_and_lokit_round_trip(
    ...
    sanitized = [
        {
            "source": "A{LOKIT_P1}B{LOKIT_P2}C{LOKIT_P3}D",
            "target": "W{LOKIT_P1}X{LOKIT_P2}Y{LOKIT_P3}Z",
        }
    ]
...
def test_duplicate_xliff_ids_are_unique_across_resources_and_survive_split_export(
    ...
    native_document = lokit.parse.xliff(
        str(path),
        progress=False,
...
async def test_sync_stream_and_async_dict_projections_are_equal(tmp_path: Path) -> None:
    ...
    exact = list(lokit.stream.to_dict(path, target_language="de-DE"))
    assert streamed == materialized == async_streamed == async_materialized == exact
    assert streamed[0]["target"] == "Hallo"


@pytest.mark.asyncio
async def test_dict_projections_thread_placeholder_opt_out(tmp_path: Path) -> None:
    path = tmp_path / "rich.tmx"
    _write_rich_tmx(path)
    streamed = list(
    ...
    materialized = lokit.parse.to_dict(
        path,
        target_language="fr-FR",
        runtime_placeholders=False,
        inline_placeholders=False,
    ...
    async_streamed = [
        row
        async for row in lokit.stream.async_.to_dict(
            path,
            target_language="fr-FR",
            runtime_placeholders=False,
            inline_placeholders=False,
    ...
    async_materialized = await lokit.parse.async_.to_dict(
        path,
        target_language="fr-FR",
        runtime_placeholders=False,
        inline_placeholders=False,
    ...
    assert streamed[0]["source"] == "Hello world !"
    assert streamed[0]["target"] == "Bonjour"
