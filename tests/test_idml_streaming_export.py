...
async def test_idml_async_cancellation_quiesces_worker_and_cleans_output(
    ...
    def blocking_rewrite(
        ...
        del source_reader, target_writer, plans
        started.set()
        while cancellation is not None and not cancellation.wait(0.005):
            pass
        raise_if_cancelled(cancellation)

    monkeypatch.setattr(idml_exporter, "_stream_rewrite_story", blocking_rewrite)
