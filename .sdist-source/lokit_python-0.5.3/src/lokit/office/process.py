    ...
    with _worker_request(options, cancellation) as session:
        ...
        try:
            for unit_id, data in items:
                ...
                bytes_seen += _translation_size(unit_id, data, target_locale)
...
def _translation_size(unit_id: str, data: Data, target_locale: str | None) -> int:
    target = data.target
    if target_locale and target_locale in data.targets:
        target = data.targets[target_locale].text
    return len(unit_id.encode("utf-8")) + len(data.source.encode("utf-8")) + len((target or "").encode("utf-8"))
