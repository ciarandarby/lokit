...
_MAX_OUTPUT_FILENAME_BYTES = 255
...
class _TranslationSpool:
    ...
    def __init__(
        ...
        try:
            ...
            self._load(translations, target_locale, options, cancellation)
    ...
    def _load(
        ...
        try:
            ...
            raw_row: object = self._connection.execute("SELECT COUNT(*) FROM translations").fetchone()
            if not isinstance(raw_row, tuple) or not raw_row or not isinstance(raw_row[0], int):
                raise OfficeReinsertionError("Office translation spool returned an invalid count")
            self._row_count = raw_row[0]
        except BaseException:
...
def _target_output_names(
    ...
    for locale in target_locales:
        if not locale or not locale.strip():
            raise OfficeReinsertionError("Office target locale filenames cannot be empty")
        if locale in {".", ".."} or locale.endswith((" ", ".")):
            raise OfficeReinsertionError(f"Unsafe Office target locale filename: {locale!r}")
        if any(ord(character) < 32 or ord(character) == 127 for character in locale):
            raise OfficeReinsertionError(f"Unsafe Office target locale filename: {locale!r}")
        if any(character in _WINDOWS_FORBIDDEN_FILENAME_CHARACTERS for character in locale):
            raise OfficeReinsertionError(f"Unsafe Office target locale filename: {locale!r}")
        reserved_stem = locale.split(".", 1)[0].casefold()
        if reserved_stem in _WINDOWS_RESERVED_FILENAMES:
            raise OfficeReinsertionError(f"Reserved Office target locale filename: {locale!r}")
        filename = f"{locale}.{file_format}"
        try:
            filename_byte_lengths = (
                len(filename.encode("utf-8")),
                len(unicodedata.normalize("NFC", filename).encode("utf-8")),
                len(unicodedata.normalize("NFD", filename).encode("utf-8")),
            )
        except UnicodeEncodeError as exc:
            raise OfficeReinsertionError("Office target locale filename is not valid UTF-8") from exc
        if max(filename_byte_lengths) > _MAX_OUTPUT_FILENAME_BYTES:
            raise OfficeReinsertionError(
                f"Office target locale filename exceeds {_MAX_OUTPUT_FILENAME_BYTES} UTF-8 bytes"
            )
        ...
        if collision_key in collision_keys:
            raise OfficeReinsertionError("Office target locales produce colliding Unicode/case-insensitive filenames")
    ...
    try:
        ...
        if _use_worker():
            ...
            try:
                ...
                units_written = translations.row_count
            finally:
                translations.close()
        raise_if_cancelled(cancellation)
