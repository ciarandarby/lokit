...
def _apply_unit_id_marker(entry: polib.POEntry, unit_id: str, unit: Data) -> None:
    ...
    # types-polib declares ``msgctxt`` as ``str`` even though polib uses
    # ``None`` for an unset context. Store it as object so mypyc does not emit
    # an invalid runtime str type-check before this sentinel comparison.
    msgctxt: object = entry.msgctxt
