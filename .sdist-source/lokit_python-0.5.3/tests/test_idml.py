...
def test_idml_roundtrip(sample_idml_file: Path, tmp_path: Path) -> None:
    ...
    assert p1.source == "This is {LOKIT_P1}bold text{LOKIT_P2}"
    ...
    unprojected = import_idml(
        str(sample_idml_file),
        source_locale="en",
        target_locale="fr",
        progress=False,
    ...
    assert unprojected.data["Story_u123:p1"].source == "This is bold text"

    imported.data["Story_u123:p0"].target = "Bonjour le monde"
    p1.target = "{LOKIT_P1}C'est du texte en gras{LOKIT_P2}"
    ...
    assert reparsed.data["Story_u123:p1"].source == "{LOKIT_P1}C'est du texte en gras{LOKIT_P2}"
