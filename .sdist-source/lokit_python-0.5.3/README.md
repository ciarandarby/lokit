...
### Runtime and inline placeholders

Every parse, stream, async, and detected-file entry point projects runtime variables and native inline codes by default. Ordered, collision-safe tokens such as `{LOKIT_P1}` make the strings safe to send to machine translation while the exact original code and its position remain in the unit's tag graph:

```python
from lokit.placeholders import PlaceholderSyntax

document = lokit.stream.po(
    "path/to/messages.po",
    placeholder_syntaxes=(PlaceholderSyntax.PYTHON_BRACE,),
)

# Restore the original variables/tags (the default) or deliberately keep the
# generic tokens as literal output for an MT-oriented intermediate file.
document.export.xliff("path/to/restored.xliff")
document.export.xliff("path/to/projected.xliff", resolve_placeholders=False)
```

Set `runtime_placeholders=False` or `inline_placeholders=False` to opt out independently. The Rust scanner recognizes Gettext format families as well as ICU MessageFormat, Unicode MessageFormat 2, Fluent, Mustache/Handlebars, ECMAScript templates, shell parameters, Swift interpolation, Rust formatting, and related standard syntaxes. Translation-memory matches compare a canonical placeholder graph and, when compatible, reform the returned translation to the query source's current placeholder names.
