...
from lokit.documents.models import DocumentSink, DocumentSource, OfficeExportResult, OfficeWarning
from lokit.documents.office.process import extract_with_worker, reinsert_with_worker, worker_available
from lokit.documents.office.runtime import load_runtime_info
...
class OfficeBackend:
    def extract(
        ...
        with _materialize_source(source, f".{file_format}", opts) as source_file:
            if _use_worker():
                _fingerprint, items = extract_with_worker(
                    source_file.path,
                    file_format,
                    source_locale,
                    target_locale,
    ...
    def stream(
        ...
        items = self.extract(source, file_format, source_locale, target_locale, opts)
...
def stream_docx(
    ...
    *,
    ...
    return _BACKEND.stream(source, "docx", source_locale, target_locale, options)
...
async def export_docx_async(
    ...
    *,
    ...
    return await asyncio.to_thread(export_docx, document, output, source_docx=source_docx, target_locale=target_locale, options=options)
...
def stream_pptx(
    ...
    *,
    ...
    return _BACKEND.stream(source, "pptx", source_locale, target_locale, options)
...
async def export_pptx_async(
    ...
    *,
    ...
    return await asyncio.to_thread(export_pptx, document, output, source_pptx=source_pptx, target_locale=target_locale, options=options)
...
def _extract_units(
    ...
    with zipfile.ZipFile(path, "r") as zf:
        ...
        if actual != file_format:
            raise OfficeUnsupportedPackageError(
                f"Expected {file_format.upper()} package, detected {actual.upper()}"
            )
        if file_format == "docx":
            parts = _docx_parts(names, options)
        else:
            parts = _pptx_parts(zf, names, options)
...
def _detect_ooxml_format(zf: zipfile.ZipFile, names: set[str]) -> str:
    ...
    if any(content_type in DOCX_MAIN_TYPES for content_type in content_types.values()) or "word/document.xml" in names:
        return "docx"
    if any(content_type in PPTX_MAIN_TYPES for content_type in content_types.values()) or "ppt/presentation.xml" in names:
        return "pptx"
...
def _docx_parts(names: set[str], options: OfficeImportOptions) -> list[str]:
    ...
    if options.include_headers_footers:
        parts.extend(sorted(name for name in names if name.startswith(("word/header", "word/footer")) and name.endswith(".xml")))
...
def _pptx_parts(zf: zipfile.ZipFile, names: set[str], options: OfficeImportOptions) -> list[str]:
    ...
    parts = slides if slides else sorted(name for name in names if name.startswith("ppt/slides/slide") and name.endswith(".xml"))
    if options.include_notes:
        parts.extend(sorted(name for name in names if name.startswith("ppt/notesSlides/notesSlide") and name.endswith(".xml")))
    if options.include_master_layout_content:
        parts.extend(sorted(name for name in names if name.startswith(("ppt/slideLayouts/", "ppt/slideMasters/")) and name.endswith(".xml")))
    return parts
...
def _extract_docx_part(
    ...
    for paragraph in root.iter(f"{{{WORD_NS}}}p"):
        ...
        units.append(_OfficeUnit(unit_id, _office_data(text, "docx", part, container, fingerprint), part, paragraph_index))
        paragraph_index += 1
        ...
        with zipfile.ZipFile(output_path, "w", compression=zipfile.ZIP_DEFLATED) as zout:
            for info in zin.infolist():
                ...
                if info.filename in parts:
                    data = _rewrite_xml_part(data, info.filename, file_format, translations, consumed, options, warnings)
...
def _rewrite_xml_part(
    ...
    for paragraph in root.iter(paragraph_tag):
        ...
        if replacement is not None:
            ...
            source = _docx_paragraph_text(paragraph) if file_format == "docx" else _pptx_paragraph_text(paragraph)
            if source.strip():
                warnings.append(OfficeWarning("office.missing_translation", f"Missing translation for {unit_id}", unit_id, part))
        paragraph_index += 1
