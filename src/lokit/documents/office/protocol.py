...
from lokit.documents.errors import OfficeProtocolError
from lokit.documents.office.runtime import PROTOCOL_MAJOR, PROTOCOL_MINOR
...
def decode_frame(data: bytes, max_frame_bytes: int = 16 * 1024 * 1024) -> ProtocolFrame:
    ...
    magic, major, minor, frame_type, flags, request_id, payload_len = HEADER.unpack(
        data[: HEADER.size]
    )
    if magic != MAGIC:
